<template>
    <admin-layout>
        ...
        <div>
            <v-btn class="mt-4" color="primary" @click="copyText">Form Register,Copy Link!</v-btn>
        </div>
...
<script>
import AdminLayout from "../../layouts/admin/CompanyLayout.vue";
...
export default {
    ...
    methods: {
        updateData() {
            ...
            this.$inertia.get("/admin/company", this.params, {
                preserveState: true,
                preverseScroll: true,
                onSuccess: () => {
        ...
        destroy() {
            this.form.delete(route("admin.company.destroy", this.itemId), {
                preverseScroll: true,
                onSuccess: () => {
        ...
        submit() {
            if (this.isUpdate) {
                this.form.put(route("admin.company.update", this.itemId), {
                    preverseScroll: true,
                    onSuccess: () => {
        ...
        copyText() {
            const textToCopy = window.location.origin + "/register/company/?incubator="+this.$page.props.incubator_key; // Replace with the text you want to copy
